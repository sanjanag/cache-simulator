...
void updateToLRU(int pos, vector<vector<int> > &lrulist, search_key* key)
{
...
void LIP(LRU_objects* lru, int state, unsigned addr)
{
  ...
  if(result1 == -1)
    {
      ...
      updateToLRU(pos1, lru->L1_lruList, key1);
      ...
      if(result2 == -1)
        {
          ...
          updateToLRU(pos2,lru->L2_lruList,key2);
          ...
          if(result3 == -1)
            {
              ...
              updateToLRU(pos3, lru->L3_lruList,key3);
            }
          else
            {
              lru->stats.L_hit++;
              ...
              updateToLRU(result3, lru->L3_lruList,key3);
      ...
      else
        {
          ...
          updateToLRU(result2, lru->L2_lruList,key2);
  ...
  else
    {
      ...
      updateToLRU(result1, lru->L1_lruList,key1);
    }
  delete key1;
  delete key2;
  delete key3;
